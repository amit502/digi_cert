...
def create_badge_section(config):
    ...
    if config.issuer_signature_lines:
        ...
        for signature_line in config.issuer_signature_lines:
            signature_image_path = os.path.join(config.abs_data_dir,config.issuer_signature_file)#signature_line['signature_image'])
...
def get_config():
    cwd = os.getcwd()
    config_file_path = os.path.join(cwd, 'cert_tools/conf.ini')
